...
class walletServicesClass {
    ...
    create = async ( data: any, userData : userAuthType ) => {
        const wallet = await this.walletMethods.create(userData);
    ...
    incrementBalance = async (data: any, userData : userAuthType) => {
        const wallet = await this.walletMethods.incrementBalance(data, userData);
        return wallet;
    }

    decrementBalance = async (data: any, userData : userAuthType) => {
        const wallet = await this.walletMethods.decrementBalance(data, userData);
        return wallet;
    }
}
